...
- Name: tests/auth.test.ts >> Authentication & Authorization Tests >> Session Scenarios >> ✅ Session persistence - Sayfa yenilemesinde session korunması
- Location: tests/auth.test.ts:163:9
...
Locator: locator('button:has-text("Çıkış"), a:has-text("Çıkış")').first()
Expected: visible
Timeout: 3000ms
Error: element(s) not found

Call log:
  - Expect "toBeVisible" locator('button:has-text("Çıkış"), a:has-text("Çıkış")').first() with timeout 3000ms
  - waiting for locator('button:has-text("Çıkış"), a:has-text("Çıkış")').first()
