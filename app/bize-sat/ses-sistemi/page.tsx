...
export default function AudioSystemPage() {
  ...
  useEffect(() => {
    ...
    // localStorage'dan kaydedilmiş form verilerini yükl
    const savedFormData = localStorage.getItem('audioSystemFormData');
    if (savedFormData) {
          // JWT token al
    ...
    try {
        ...
        console.log('📝 Kaydedilmiş ses sistemi form verileri yüklendi');
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!isLoggedIn) {
      router.push('/login?returnUrl=' + encodeURIComponent('/bize-sat/ses-sistemi'));
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        setPopupMessage('Ses sistemi ürününüz için teklif talebiniz alındı. En kısa sürede size dönüş yapacağız.');
        setShowPopup(true);
        
        localStorage.removeItem('audioSystemFormData');
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {/* Login Required Card */}
        {!isLoggedIn && <LoginRequiredCard isMobile={isMobile} returnUrl="/bize-sat/ses-sistemi" />}.map((_, index) => (
