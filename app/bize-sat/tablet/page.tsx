...
export default function TabletPage() {
  ...
  useEffect(() => {
    ...
    const savedFormData = localStorage.getItem('tabletFormData');
    if (savedFormData) {
          // JWT token al
    ...
    try {
        ...
        console.log('📝 Kaydedilmiş tablet form verileri yüklendi');
      } catch (error) {
  ...
  const handleInputChange = (field: string, value: any) => {
    ...
    localStorage.setItem('tabletFormData', JSON.stringify(newData));
  };

  
  // Hızlı resim sıkıştırma fonksiyonu (senkron)
  const compressImageSync = (base64String: string): string => {
    try {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!isLoggedIn) {
      router.push('/login?returnUrl=' + encodeURIComponent('/bize-sat/tablet'));
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        setPopupMessage('Tablet ürününüz için teklif talebiniz alındı. En kısa sürede size dönüş yapacağız.');
        setShowPopup(true);
        
        localStorage.removeItem('tabletFormData');
      ...
      <div style={{
        ...
        <div style={{
          ...
          <p style={{
            ...
            Tablet'inizi satın, en iyi fiyatı alın
          </p>
        </div>

        {/* Login Required Card */}
        {!isLoggedIn && <LoginRequiredCard isMobile={isMobile} returnUrl="/bize-sat/tablet" />}
            ...
            <div style={{
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Örn: iPad Pro, Galaxy Tab, MatePad"
                  style={{
                    width: '100%',
                    padding: '12px',
                    border: '1px solid #d1d5db',
        ...
        <div style={{
          ...
          <div style={{
            ...
            <p style={{
              ...
              Tablet'iniz için teklif talebiniz alındı. En kısa sürede size ulaşacağız.
            </p>
            <p style={{
              fontSize: '13px',
              color: '#059669',
