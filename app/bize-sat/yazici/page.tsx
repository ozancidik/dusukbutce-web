...
import PrinterBasicInfo from './components/PrinterBasicInfo';
import PrinterTechnicalSpecs from './components/PrinterTechnicalSpecs';
import PrinterCondition from './components/PrinterCondition';
import PrinterImages from './components/PrinterImages';
import PrinterSubmit from './components/PrinterSubmit';
...
export default function YaziciPage() {
  ...
  useEffect(() => {
    ...
    try {
      const savedFormData = localStorage.getItem('printerFormData');
      if (savedFormData) {
      ...
      try {
        localStorage.removeItem('printerFormData');
      } catch (innerError) {
  ...
  const handleInputChange = (field: string, value: any) => {
    setFormData(prev => {
      ...
      try {
        localStorage.setItem('printerFormData', JSON.stringify(newData));
      } catch (error) {
        console.warn('localStorage quota hatası:', error);
        const dataWithoutImages = { ...newData, images: [] };
        try {
          localStorage.setItem('printerFormData', JSON.stringify(dataWithoutImages));
        } catch (innerError) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!isLoggedIn) {
      router.push('/login?returnUrl=' + encodeURIComponent('/bize-sat/yazici'));
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        setPopupMessage('Yazıcınız için teklif talebiniz alındı. En kısa sürede size dönüş yapacağız.');
        setShowPopup(true);
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <PrinterHeader isMobile={isMobile} />
        ...
        <form onSubmit={handleSubmit} style={{
          ...
          <div style={{
            ...
            ⚠️ <strong>Önemli:</strong> Sadece çalışır durumda olan yazıcılar satın alınır. Kartuş/toner durumu fiyatı etkiler.
          </div>
