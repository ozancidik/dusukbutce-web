...
export default function BuyDesktopPage() {
  // State'ler
  ...
  const [powerSupplyValue, setPowerSupplyValue] = useState('');
  ...
  const isFormValid = useMemo(() => {
    ...
    if (!graphicsCardValue || graphicsCardValue.trim().length < 2) return false;
    if (!processorValue || processorValue.trim().length < 2) return false;
    if (!storageValue || storageValue.trim().length < 2) return false;
    if (!ramValue || ramValue.trim().length < 2) return false;
    if (!powerSupplyValue || powerSupplyValue.trim().length < 2) return false;
    if (!motherboardValue || motherboardValue.trim().length < 2) return false;
    return true;
  }, [brandValue, modelValue, graphicsCardValue, processorValue, storageValue, ramValue, powerSupplyValue, motherboardValue]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const formData = {
        ...
        powerSupply: powerSupplyValue,
        motherboard: motherboardValue,
        images: images.filter(img => img !== null)
      };

      const response = await fetch('/api/desktop-submissions', {
        method: 'POST',
        headers: {
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: isMobile ? '16px' : '24px' }}>
            ...
            <div>
              ...
              <input
                type="text"
                placeholder="Örn: Intel i5-12400, AMD Ryzen 5 5600..."
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: isMobile ? '16px' : '24px' }}>
            ...
            <div>
              ...
              <input
                type="text"
                placeholder="Örn: ROG, Inspiron, Pavilion..."
                value={modelValue}
            ...
            <div>
              ...
              <input
                type="text"
                placeholder="Örn: 550W, 750W, 850W..."
                value={powerSupplyValue}
                onChange={(e) => setPowerSupplyValue(e.target.value)}
                style={{
                  width: '100%',
                  padding: isMobile ? '14px 16px' : '16px 20px',
                  border: '1px solid #e2e8f0',
            ...
            <div>
              ...
              <input
                type="text"
                placeholder="Örn: B660, X570, Z690..."
                value={motherboardValue}
                onChange={(e) => setMotherboardValue(e.target.value)}
                style={{
                  width: '100%',
                  padding: isMobile ? '14px 16px' : '16px 20px',
                  border: '1px solid #e2e8f0',
