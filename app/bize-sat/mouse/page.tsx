...
export default function MousePage() {
  ...
  useEffect(() => {
    ...
    const savedFormData = localStorage.getItem('mouseFormData');
    if (savedFormData) {
          // JWT token al
    ...
    try {
        ...
        console.log('📝 Kaydedilmiş mouse form verileri yüklendi');
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!isLoggedIn) {
      router.push('/login?returnUrl=' + encodeURIComponent('/bize-sat/mouse'));
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        setPopupMessage('Mouse ürününüz için teklif talebiniz alındı. En kısa sürede size dönüş yapacağız.');
        setShowPopup(true);
        
        localStorage.removeItem('mouseFormData');
      ...
      <div style={{
        ...
        <div style={{
          ...
          <p style={{
            ...
            Mouse'unuzu satın, en iyi fiyatı alın
          </p>
        </div>

        {/* Login Required Card */}
        {!isLoggedIn && <LoginRequiredCard isMobile={isMobile} returnUrl="/bize-sat/mouse" />}
