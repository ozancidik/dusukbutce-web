...
export default function RamPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      try {
        localStorage.removeItem('ramFormData');
      } catch (innerError) {
  ...
  const handleInputChange = (field: string, value: any) => {
    setFormData(prev => {
      ...
      try {
        localStorage.setItem('ramFormData', JSON.stringify(newData));
      } catch (error) {
        ...
        try {
          localStorage.setItem('ramFormData', JSON.stringify(dataWithoutImages));
        } catch (innerError) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {/* Login Required Card */}
        {!isLoggedIn && <LoginRequiredCard isMobile={isMobile} returnUrl="/bize-sat/ram" />}.map((_, index) => (
