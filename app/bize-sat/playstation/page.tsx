...
export default function PlayStationPage() {
  ...
  useEffect(() => {
    ...
    try {
      const savedFormData = localStorage.getItem('playstationFormData');
      if (savedFormData) {
      ...
      try {
        localStorage.removeItem('playstationFormData');
      } catch (innerError) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!isLoggedIn) {
      router.push('/login?returnUrl=' + encodeURIComponent('/bize-sat/playstation'));
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        setPopupMessage('PlayStation konsolunuz için teklif talebiniz alındı. En kısa sürede size dönüş yapacağız.');
        setShowPopup(true);
  ...
  const handleInputChange = (name: string, value: any) => {
    ...
    setFormData(prev => {
      ...
      try {
        localStorage.setItem('playstationFormData', JSON.stringify(newData));
      } catch (error) {
        ...
        try {
          localStorage.setItem('playstationFormData', JSON.stringify(dataWithoutImages));
        } catch (innerError) {
      ...
      <div style={{
        ...
        <div style={{
          ...
          <p style={{
            ...
            PlayStation konsolunuzu değerinde satın, hızlı ödeme alın
          </p>
        </div>

        {/* Login Required Card */}
        {!isLoggedIn && <LoginRequiredCard isMobile={isMobile} returnUrl="/bize-sat/playstation" />}
        ...
        <form onSubmit={handleSubmit} style={{
          ...
          <div style={{ marginBottom: isMobile ? '20px' : '24px' }}>
            ...
            >
              ...
              <option value="PS5 Standart">PlayStation 5 Standart</option>
          ...
          <div>
            ...
            >
              ...
              <option value="Çok İyi">Çok İyi (Çok az çizik)</option>
          ...
          {/* Fatura Tarihi - Sadece hasInvoice true ise göster */}
          {formData.hasInvoice && (
            <div style={{
              gridColumn: isMobile ? '1' : '1 / -1',
              display: 'grid',
